 ...
 *
 * @global string msgSeemsDeletedMoved
 * @param  object $form
 * @param  string response
 * @return void
 */
function manageSuccessResponse($form, response) {
  ...
  if (typeof msgSeemsDeletedMoved !== 'undefined') {
    $markup += '<tr id="record-0">';
    $markup += '<td colspan="' + $('.records-thead th').length + '">';
    $markup += msgSeemsDeletedMoved;
    $markup += '</td>';
    $markup += '</tr>';
  }
...
function stripeTokenHandler(result) {
  ...
  if (typeof result.token !== 'undefined') {
    $('#stripe-pay-form').find('[name="stripe_token"]').val(result.token.id);
    form.submit();
  }
  else {
    hideStripeError();
...
function showStripeError(result) {
  $('.card-errors').html('<p>' + result.error.message + '</p>');
