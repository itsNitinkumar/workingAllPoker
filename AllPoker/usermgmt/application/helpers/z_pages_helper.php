...
/**
 * Interface ( API, Direct ) Key
 *
 * @param   integer $interface
 * @return  string
 * @version 1.6
 */
if ( ! function_exists( 'interface_key' ) )
{
    function interface_key( $interface )
    {
        if ( $interface == 2 ) return 'api';
        else return 'direct';
    }
}
...
/**
 * Get Body "<body>" Class(s) ( Depending on Area ).
 *
 * @return string
 */
 ...
 */
if ( ! function_exists( 'menu_pending' ) )
{
    function menu_pending( $type, $count = false )
    {
        $ci =& get_instance();
        ...
        {
            case 'user_ticket';
                $count = $ci->Support_model->pending_ticket_user( $user_id );
                break;
              
            case 'admin_ticket':
                $count = $ci->Support_model->pending_ticket_admin();
                break;
              
            case 'contact_message':
                $count = $ci->Support_model->not_read_cm();
                break;
        }
...
/**
 * Get Limited Announcements
 *
 * @param  integer $limit
 * @return object
 */
if ( ! function_exists( 'get_limited_announcements' ) )
{
    function get_limited_announcements( $limit )
    { 
        $ci =& get_instance();
        return $ci->Tool_model->announcements( false, $limit );
    }
}
