...
class Users extends MY_Controller {
     ...
     * @version 1.6
     */
    public function api_key( $action = '' )
    {
        if ( empty( $action ) ) r_error( 'invalid_req' );
        ...
        if ( $status )
        {
            if ( $action === 'generate' )
            {
                $key = 'user_api_key_generated';
            }
            else
            {
                $key = 'user_api_key_reset';
            }
            
            log_user_activity( $key, $id );
            r_s_jump( $togo );
        }
    ...
    public function delete_profile_picture()
    {
        ...
        {
            log_user_activity( 'u_pp_deleted_by_admin', $id );
    ...
    public function new_user()
    {
        if ( $this->form_validation->run( 'new_user' ) )
        {
            ...
            if ( is_string( $cf_status ) ) r_error( $cf_status );
            
            $id = $this->User_model->add( $data );
            
            if ( ! empty( $id ) )
            {
                log_user_activity( 'user_registered_by_admin', $id );
    ...
    public function update_user()
    {
        if ( $this->form_validation->run( 'profile_settings' ) )
        {
            ...
            if ( $status === true )
            {
                log_user_activity( 'user_profile_updated', $id );
    ...
    public function delete_user()
    {
        ...
        if ( delete_user( $id ) )
        {
            log_user_activity( 'user_deleted', $id );
            r_success_remove( $id );
        }
    ...
    public function send_email_user()
    {
        if ( $this->form_validation->run( 'send_email_user' ) )
        {
            ...
            $sender_comment = do_secure( post( 'message' ) );
            ...
            if ( $this->zmailer->send_email( $email_address, $subject, $message ) )
            {
                ...
                log_user_activity( 'user_sent_email', $email_address );
    ...
    public function delete_sent_email()
    {
        ...
        {
            log_user_activity( 'u_sent_email_deleted', $id );
            r_success_remove( $id );
        }
    ...
    public function impersonate()
    {
        ...
        if ( $this->zuser->has_permission( 'impersonate' ) )
        {
            ...
            {
                if ( ! empty( $this->User_model->get_by_id( $id ) ) )
                {
                    log_user_activity( 'impersonate', $id );
    ...
    public function deimpersonate()
    {
        if ( get_session( 'impersonating' ) )
        {
            ...
            log_user_activity( 'deimpersonate', $tmp_uid, $admin_id );
    ...
    public function send_vlink()
    {
        ...
        if ( $status === true )
        {
            log_user_activity( 'sent_vlink', $id );
    ...
    /**
     * Adjust Balance Page Input Handling.
     *
     * @return void
     */
    public function adjust_balance()
    {
        ...
        if ( $this->form_validation->run( 'adjust_balance' ) )
        {
            $type = do_secure( post( 'type' ) );
            
            if ( ! in_array( $type, ['add_credit', 'cut_credit'] ) ) r_error( 'invalid_req' );
            
            $data = [
                'user_id' => intval( post( 'user_id' ) ),
                'currency_id' => intval( post( 'currency' ) ),
                'item_name' => ucwords( str_replace( '_', ' ', $type ) ),
                'visible_to_user' => only_binary( post( 'visible_to_user' ) ),
                'create_invoice' => only_binary( post( 'create_invoice' ) ),
                'gateway' => do_secure_l( post( 'payment_gateway' ) ),
                'amount' => do_secure( post( 'amount' ) )
            ];
            ...
            if ( $type === 'cut_credit' )
            {
                $row = $this->User_model->credit( $data['user_id'], $data['currency_id'] );
                
                if ( ! empty( $row ) )
                {
                    if ( ( doubleval( $row->credit ) - $data['amount'] ) < 0 )
                    {
                        r_error( 'cant_cut_zero' );
                    }
                }
                else
                {
                    r_error( 'cant_cut_currency' );
                }
            }
            
            $status = $this->User_model->{$type}( $data['user_id'], $data['currency_id'], $data['amount'] );
            ...
            if ( $status )
            {
                ...
                log_user_activity( 'balance_adjusted', $data['user_id'] );
                r_s_jump( "admin/users/adjust_balance/{$data['user_id']}", 'balance_adjusted' );
            }
            
            r_error( 'went_wrong' );
        }
        
        d_r_error( form_error( 'amount' ) );
    }
        ...
        if ( $this->form_validation->run( 'user_invite' ) )
        {
            ...
            if ( $this->zmailer->send_email( $data['email_address'], $template->subject, $message ) )
            {
                ...
                if ( ! empty( $id ) )
                {
                    ...
                    log_user_activity( 'user_invited', $id );
    ...
    public function update_invitation()
    {
        if ( $this->form_validation->run( 'user_invite' ) )
        {
            ...
            if ( $this->User_model->update_invitation( $data, $id ) )
            {
                ...
                log_user_activity( 'user_invitation_updated', $id );
    ...
    public function delete_invitation()
    {
        ...
        {
            log_user_activity( 'user_invitation_deleted', $id );
            r_success_remove( $id );
        }
