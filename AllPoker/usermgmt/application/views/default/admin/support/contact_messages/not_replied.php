      ...
      <div class="col-sm-12">
        ...
        <div class="card">
          <div class="card-header d-flex align-items-center">
            <h3 class="card-title"><?php echo lang( 'not_replied_msgs' ); ?></h3>
            <div class="card-tools ml-auto">
              <a href="<?php echo env_url( 'admin/support/contact_messages/replied' ); ?>" class="btn btn-dark text-sm">
                <i class="fas fa-envelope-open mr-2"></i> <?php echo lang( 'replied_msgs' ); ?>
              </a>
            </div>
            <!-- /.card-tools -->
          </div>
              ...
              <table class="custom-table z-table table table-striped text-nowrap table-valign-middle mb-0">
                <thead class="records-thead">
                  <tr>
                    ...
                    <th class="text-right"><?php echo lang( 'received' ); ?></th>
                  ...
                  if ( ! empty( $messages ) )
                  {
                    foreach ( $messages as $message ) {
                      ...
                      <tr id="record-<?php echo html_escape( $id ); ?>">
                        ...
                        <td>
                          <?php if ( $message->is_read ) echo html_escape( short_text( $message->message ) ); ?>
                          
                          <?php if ( $message->is_read == 0 || is_increased_short_text( $message->message ) ) { ?>
                            <span class="badge badge-success get-data-tool" data-source="<?php admin_action( 'support/contact_message/message' ); ?>" data-id="<?php echo html_escape( $id ); ?>">
                              <?php echo lang( 'read_message' ); ?>
                            </span>
                            
                            <?php if ( $message->is_read == 0 ) { ?>
                               <span class="ml-1 badge badge-danger"><?php echo lang( 'new' ); ?></span>
                            <?php } ?>
                        ...
                        <td class="text-right">
                          <div class="btn-group">
                            <button class="btn btn-sm btn-primary get-data-tool" data-source="<?php admin_action( 'support/reply_box' ); ?>" data-id="<?php echo html_escape( $id ); ?>">
                              <span class="fas fa-reply get-data-tool-c"></span>
                            </button>
