...
connections.on("connection", async (socket) => {
    ...
    const removeItems = (items, id, type) => {
    ...
    socket.on("createWebRtcTransport", async ({ consumer }, callback) => {
        console.log(`Is consumer: ${consumer}`);

        const roomName = peers[socket.id].roomName

        // console.log("peers", peers);
        // console.log("peers[socket.id]", peers[socket.id]);
        // console.log("createWebRtcTransport", roomName);
    ...
    const addTransport = (transport, roomName, consumer) => {
        console.log("addTransport", transport.id)
        transports = [
            ...transports,
    ...
    const informConsumers = (roomName, socketId, producerId) => {
        console.log(`informConsumers ${roomName}, ${socketId}, ${producerId}`);
    ...
    const getTransport = (socketId) => {
        const producerTransport = transports.find(transport => transport.socketId === socketId && !transport.consumer)

        // console.log("getTransport", producerTransport.transport);
    ...
    socket.on("transport-recv-connect", async ({ dtlsParameters, serverConsumerTransportId }) => {
        ...
        // console.log("transports", transports[1].transport.id)
        ...
        try {
            ...
            if (router.canConsume({ producerId: remoteProducerId, rtpCapabilities })) {
                ...
                consumer.on("producerclose", () => {
                    ...
                    transports = transports.filter(transport => transport.transport.id !== serverConsumerTransportId)
                    consumer.close()
                    consumers = consumers.filter(consumer => consumer.producer.id !== producer.id)
                })
